...
//database connection
mongoose
  .connect("mongodb://127.0.0.1:27017/AuthenticationEx")
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file.mimetype == "image/png" || file.mimetype == "image/jpeg") {
      ...
      cb(null, false);
      return cb(new Error("only png and jpg format support"));
    }
  },
});
...
app.post("/postlogin", (req, res) => {
  ...
  if(!name1.test(uname)) {
    unameErr = 'Must be letters only ';
}
if(!pass1.test(password)) {
    passErr = 'password must be between 8 to 24 characters which contain at least one  uppercase,lowercase'
}
